/*
 * kopouSwitch library v1.2.0 (20140128) 
...
typedef void (*kopouReceiverCallBack)(kopouCode);

/**
* See RemoteSwitch for introduction.
*
* kopouReceiver decodes the signal received from a 433MHz-receiver, like the "KlikAanKlikUit"-system
...
* A call to the callback must be finished before kopouReceiver will call the callback function again, thus
* there is no re-entrant problem.
*
* When sending your own code using kopouSwich, disable() the receiver first.
*
...
class kopouReceiver {
	public:
		...
		* @param callback Pointer to a callback function, with signature void (*func)(kopouCode)
		*/
		static void init(int8_t interrupt, byte minRepeats, kopouReceiverCallBack callback);
	...
	private:
		...
		static kopouReceiverCallBack _callback;
