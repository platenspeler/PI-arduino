...
// Do the actual reading of the BH1750 sensor
// 
uint16_t BH1750FVI::GetLightIntensity(void){
  ...
  if (Intensity_value == (int)-1) {			// Value == oxFFFF == -1
	  return (Intensity_value);
  }
  Intensity_value=Intensity_value/1.2;		// Will never be (int) -1 again
  return Intensity_value; 
}

// For writing init values 
