...
typedef void (*KakuReceiverCallBack)(KakuCode);

/**
* See RemoteSwitch for introduction.
*
* KakuReceiver decodes the signal received from a 433MHz-receiver, like the "KlikAanKlikUit"-system
...
* A call to the callback must be finished before KakuReceiver will call the callback function again, thus
...
class KakuReceiver {
	public:
		...
		* @param callback Pointer to a callback function, with signature void (*func)(KakuCode)
		*/
		static void init(int8_t interrupt, byte minRepeats, KakuReceiverCallBack callback);
	...
	private:
		...
		static KakuReceiverCallBack _callback;
